...
func DeleteUnusedImages(ctx context.Context, registry types.RegistrySettings, usedImages []string) (finalError error) {
	...
	for _, r := range searchResult {
		...
		if err != nil {
			logger.Infof("failed to parse registry image ref %q: %v", imageName, err)
			continue
		}

		tags, err := docker.GetRepositoryTags(ctx, sysCtx, ref)
		if err != nil {
			logger.Infof("failed to get repo tags for %q: %v", imageName, err)
			continue
		}
		...
		for _, tag := range tags {
			...
			if err != nil {
				logger.Infof("failed to parse tagged ref %q: %v", taggedName, err)
				continue
			}
			...
			if err != nil {
				if !strings.Contains(err.Error(), "StatusCode: 404") {
					logger.Infof("failed to get digest for %q: %v", taggedName, err)
				}
				continue
			}
